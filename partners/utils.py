...
def is_valid_partner_data(data):
    if not data.get('name') or not data.get('formalName'):
        return False, JsonResponse({'message': 'Fill required fields.'}, status=400)
    if not data.get('customer') and not data.get('supplier'):
        return False, JsonResponse({'message': 'You must select at least one option, either Customer or Supplier!'}, status=400)
    return True, None
